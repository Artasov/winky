    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            if let Err(error) = check_for_updates(&app, false).await {
                log::warn!(target: "update", "Update check failed: {error}");
            }
...
async fn check_for_updates(
    app: &AppHandle,
    emit_manifest_errors: bool,
) -> Result<UpdateCheckResult> {
    if cfg!(debug_assertions) && std::env::var("WINKY_ENABLE_DEBUG_UPDATER").is_err() {
        log::debug!(
            target: "update",
            "Skipping update check in debug build without WINKY_ENABLE_DEBUG_UPDATER"
        );
    ...
    if let Err(error) =
        download_with_progress(app, &client, &candidate.file_url, &installer_path).await
    {
        emit_update_error(app, error.to_string());
...
fn resolve_manifest_url() -> String {
    std::env::var("WINKY_UPDATE_MANIFEST_URL")
        .ok()
...
fn absolutize_update_url(raw_file: &str, manifest_url: &str) -> Result<String> {
    ...
    Ok(Url::parse(manifest_url)?.join(raw_file)?.to_string())
}
...
fn file_name_from_url(file_url: &str, version: &str) -> String {
    Url::parse(file_url)
        ...
        .unwrap_or_else(|| {
            ...
            format!("winky-{}.{}", normalize_version(version), extension)
        })
}
...
fn verify_hash(path: &Path, expected_hash: &str) -> Result<()> {
    ...
    if hash != expected_hash {
        ...
        return Err(anyhow!(
            "Downloaded update hash does not match the manifest"
        ));
    }
    Ok(())
}
...
async fn install_update(installer: &Path) -> Result<()> {
    ...
    base.push("winky-updater");
    fs::create_dir_all(&base)?;
    let script_path = base.join("winky-update.bat");
