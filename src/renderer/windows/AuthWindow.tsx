...
const renderOAuthIcon = (provider: AuthProvider) => {
    ...
    if (provider === 'discord') {
        return (
            <svg width="22" height="22" fill="#5865F2" viewBox="0 0 24 24">
                <path
    ...
    return (
        <svg width="20" height="20" fill="#fc3f1d" viewBox="0 0 256 512">
            <path
...
const AuthWindow: React.FC = () => {
    ...
    useEffect(() => {
        ...
        auth.getAuthMethods()
            .then((methods) => {
                if (cancelled) return;
                const allowed = new Set(methods.allowed_oauth_providers);
                setAllowedOAuthProviders(OAUTH_PROVIDERS.filter((provider) => allowed.has(provider)));
            })
            .catch((error) => {
    ...
    return (
        <div className="fc h-full">
            ...
            <div className="relative flex-1">
                <div className="fccc mx-auto h-full w-full max-w-md px-6 py-10">
                    <div className="fc w-full gap-4">
                        <Typography variant="h4" fontWeight={700} color="text.primary">
                            Sign In
                        </Typography>

                        <Box component="form" onSubmit={handleSubmit} className="fc gap-3">
                            <TextField
                                type="email"
                                label="Email"
                                value={email}
