...
import {API_BASE_URL, AUTH_ENDPOINT, AUTH_REFRESH_ENDPOINT} from '@shared/constants';
...
function readStorage(): AuthTokens | null {
    ...
    try {
        ...
        console.warn('[authClient] Failed to read tokens from storage', error);
        return null;
    }
}
...
function writeStorage(tokens: AuthTokens | null): void {
    ...
    try {
        ...
        console.warn('[authClient] Failed to write tokens to storage', error);
    }
}
...
export class AuthClient {
    ...
    public async login(email: string, password: string): Promise<User> {
        ...
        try {
            ...
            console.log('[authClient] User signed in');
            return user;
        } catch (error) {
            this.clearTokens();
            throw normalizeError(error);
    ...
    public async getCurrentUser(includeTiersAndFeatures: boolean = false): Promise<User> {
    ...
    public async refreshAccessToken(): Promise<string | null> {
        ...
        this.refreshPromise = (async () => {
            try {
                ...
                const tokens = this.parseTokenResponse(data);
                this.updateTokens(tokens);
                console.log('[authClient] Access token refreshed');
