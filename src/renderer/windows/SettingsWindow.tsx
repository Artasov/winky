...
const SettingsWindow: React.FC = () => {
    ...
    const handleCreateAction = async (event: React.FormEvent) => {
        ...
        setSaving(true);
        console.log('%c[SettingsWindow] %cСоздание действия...', 'color: #3b82f6; font-weight: bold', 'color: inherit', {
            name: name.trim(),
            prompt: prompt.trim(),
            icon: iconId
        });
    ...
    const handleDeleteAction = async (actionId: string, actionName: string) => {
        ...
        console.log('%c[SettingsWindow] %cУдаление действия...', 'color: #3b82f6; font-weight: bold', 'color: inherit', {
            actionId,
            actionName
        });
        try {
    ...
    return (
        <div className="fc h-full p-8 gap-6 overflow-hidden">
                    ...
                    <div className="fc gap-6">
                        ...
                        <div className="fc gap-3">
                            <label className="text-sm font-medium text-slate-200">
                                Выберите иконку {iconId && <span
                                className="text-slate-400 font-normal">• {icons.find(i => i.id === iconId)?.name}</span>}
                            </label>
                            {loadingIcons ? (
                                <div
                                    className="rounded-lg border border-slate-600 bg-slate-900 px-4 py-8 text-center text-slate-400">
                                    Загрузка иконок...
                                </div>
                            ) : icons.length === 0 ? (
                                <div
            ...
            <section className="flex-1 overflow-auto rounded-lg border border-white/10 bg-white/5 p-6">
                ...
                {actions.length === 0 ? (
                    <div
                        className="fcc rounded-lg border border-dashed border-slate-700 bg-slate-900/50 py-12 text-center">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="frc gap-3 mb-2">
                                            {action.icon_details?.svg ? (
                                                ...
                                                <div
                                                    className="fcc rounded-lg bg-slate-800 p-2 text-xl shrink-0">❓</div>
                                            )}
